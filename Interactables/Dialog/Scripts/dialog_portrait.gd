...
func check_mouth_open( l : String ) -> void:
	if 'aeiouy1234567890'.contains( l ):
		open_mouth = true
		mouth_open_frames += 3
		audio_stream_player_2d.pitch_scale = randf_range( audio_pitch_base - 0.04, audio_pitch_base + 0.04 )
		audio_stream_player_2d.play()
	elif '.,!?'.contains( l ):
		audio_stream_player_2d.pitch_scale = audio_pitch_base - 0.1
		audio_stream_player_2d.play()
		mouth_open_frames = 0
