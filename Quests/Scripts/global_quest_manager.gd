#QUEST MANAGER - GLOBAL SCRIPT / AUTOLOAD
...
func _unhandled_input(event : InputEvent) -> void:
	if event.is_action_pressed("test"):
		#print( find_quest(load("res://Quests/give_food.tres") as Quest) )
		#print( find_quest_by_title( "short Quest" ) )
		#print( "get_quest_index_by_title: ",get_quest_index_by_title("Give Food") )
		#print( "get_quest_index_by_title: ",get_quest_index_by_title("short quest") )
		#print( "get_quest_index_by_title: ",get_quest_index_by_title("long quest") )
		
		#print("before: " ,current_quests )
		
		#update_quest( "short quest", "", true)
		#update_quest( "give food", "Find Food", true )
...
func gather_quest_data() -> void:
	#gather all resources and add to quests array
...
func disperse_quest_rewards( _q : Quest) -> void:
	var _message : String = str( _q.reward_exp ) + "xp"
	PlayerManager.reward_xp( _q.reward_exp )
	for i in _q.reward_items:
		PlayerManager.INVENTORY_DATA.add_item( i.item, i.quantity )
		_message += ", " + i.item.name + " x" + str( i.quantity )
	
	PlayerHud.queue_notification( "Quest Reward Recieved!", _message )
...
func get_quest_index_by_title( _title : String ) -> int:
	for i in current_quests.size():
		if current_quests[ i ].title.to_lower() == _title.to_lower():
			return i
		#Return a -1 if we didn't find a quest
